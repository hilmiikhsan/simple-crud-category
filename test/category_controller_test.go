...
func setupTestDB() *sql.DB {
	db, err := sql.Open("mysql", "root:#21012123Op@tcp(localhost:3306)/crud_category_test")
...
func TestCreateCategorySuccess(t *testing.T) {
	...
	requestBody := strings.NewReader(`{"name": "Komputer"}`)
	...
	assert.Equal(t, "Komputer", responseBody["data"].(map[string]interface{})["name"])
}
...
func TestListCategorySuccess(t *testing.T) {
	...
	categoryRespose1 := categories[0].(map[string]interface{})
	categoryRespose2 := categories[1].(map[string]interface{})

	assert.Equal(t, category1.Id, int(categoryRespose1["id"].(float64)))
	assert.Equal(t, category1.Name, categoryRespose1["name"])

	assert.Equal(t, category2.Id, int(categoryRespose2["id"].(float64)))
	assert.Equal(t, category2.Name, categoryRespose2["name"])
}
...
func TestUnauthorized(t *testing.T) {
	...
	request := httptest.NewRequest(http.MethodGet, "http://localhost:3000/api/categories", nil)
	request.Header.Add("X-API-KEY", "SECRET FALSE")
